...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i in range(len(sentence)):
            data = []
            for w in range(i-1,i-1-window_size,-1):
                if w >=0:
                    data.extend(sentence[w])
                else:
                    data.extend(start)
            data.extend(sentence[i])
            for w in range(i+1,i+1+window_size):
                if w <len(sentence):
                    data.extend(sentence[w])
                else:
                    data.extend(end)

            windowed_data.append((data,labels[i]))
		### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=1):
        ...
        feed_dict = {
            ...
            # self.labels_placeholder : labels_batch, BUG!!!
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        E = tf.Variable(self.pretrained_embeddings, name='Embeddings')
        embeddings = tf.nn.embedding_lookup(E, self.input_placeholder)
        embeddings = tf.reshape(embeddings, (-1, self.config.n_window_features*self.config.embed_size), name='embeddings')                                                                                                        
        ### END YOUR CODE
        return embeddings
